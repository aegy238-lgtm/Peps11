...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ onBack, language }) => {
  ...
  // --- SVGA Logic ---
  const handleSvgaIconUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
      const file = e.target.files?.[0]; 
      if (!file) return;
      try {
          // Compress the icon to ensure it's small enough for Firestore
          // Using 300px width and 0.8 quality
          const base64 = await compressImage(file, 300, 0.8);
          setSvgaIcon(base64);
      } catch (err) {
          alert("خطأ في معالجة الصورة، يرجى اختيار صورة أصغر.");
      }
  };
  ...
  const handleSvgaFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      // Strict size check for animations since we can't easily compress .svga files client-side without libraries
      if (file.size > 900 * 1024) { // 900KB limit (safe zone for 1MB doc limit)
          alert("حجم الملف كبير جداً! يجب أن يكون أقل من 900 كيلوبايت.");
          return;
      }
  ...
  const handleAddSvgaItem = async () => {
      ...
      try {
          ...
          if (e.message && e.message.includes("larger than")) {
              alert("فشل الرفع: حجم الملف كبير جداً لقاعدة البيانات. حاول تقليل حجم الصور.");
          } else {
